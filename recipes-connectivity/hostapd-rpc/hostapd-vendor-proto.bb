...
do_install() {
    install -d ${D}${includedir}/rpc/proto/hostapd_vendor
    install -m 0755 ${S}/a2p_gen/gen/*.pb.h ${D}${includedir}/rpc/proto/hostapd_vendor
    install -m 0755 ${S}/a2p_gen/*.proto ${D}${includedir}/rpc/proto/hostapd_vendor

    install -d ${D}${libdir}
    install -m 0755 ${S}/a2p_gen/*.so.0 ${D}/${libdir}

    cd ${D}${libdir}
    ln -s libqti-hostapd-vendor-proto.so.0 libqti-hostapd-vendor-proto.so
}
