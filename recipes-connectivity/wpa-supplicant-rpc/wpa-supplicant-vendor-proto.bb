DESCRIPTION = "QTI Wpa-supplicant Vendor Proto Library"
...
do_compile:prepend() {
    mkdir -p ${S}/a2p_gen
    a2p --aidl-path ${S}${AIDL_PATH_VENDOR} --proto-path ${S}/a2p_gen --makefile-type makefile
...
do_install() {
    install -d ${D}${includedir}/rpc/proto/wpa_supplicant_vendor
    install -m 0755 ${S}/a2p_gen/gen/*.pb.h ${D}${includedir}/rpc/proto/wpa_supplicant_vendor
    install -m 0755 ${S}/a2p_gen/*.proto ${D}${includedir}/rpc/proto/wpa_supplicant_vendor

    install -d ${D}${libdir}
    install -m 0755 ${S}/a2p_gen/*.so.0 ${D}/${libdir}

    cd ${D}${libdir}
    ln -s libqti-supplicant-vendor-proto.so.0 libqti-supplicant-vendor-proto.so
}
